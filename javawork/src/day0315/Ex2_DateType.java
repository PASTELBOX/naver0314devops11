...
public class Ex2_DateType {
	...
	public static void main(String[] args) {
		...
		byte b=(byte) 200; //(byte) : cast연산자 : 강제로 형변환, 값 손실 발생
		System.out.println(b); // 값손실이 발행할 경우 엉뚱한 값이 출력(내부적으로는 2의 보수값이다.)
		
		//실수 타입
		float f1=123.567891234f; //f를 붙여야 4 바이트 float 으로 저장
		double f2=123.567891234;
		
		System.out.println(f1);//정밀도가 보통 8자리까지 정확히 나온다
		System.out.println(f2);//정밀도가 보통 15자리까지 정확히 나온다
		
		//1글자 지정은 char, 문자열은 String(객체 타입)
		char ch1='A';//char는 '' 로 해야됨
