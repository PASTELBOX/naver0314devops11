...
class MyShop
{
	...
	{
		//static 멤버 변수는 같은 구역에 같은 이름의 변수가 있을 경우 앞에 클래스명을 생략하지 말고 써준다
	...
	//overloading method(중복함수):메서드명은 같으나 인자가 달라야 한다=>인자의 위치만 바꿔도 되고 추가,변경 전부가능
	public void setData(String sangpum,int su,int dan,String message)
	{
		...
		//반대로 static 메서드에서 인스턴스 멤버 메서드를 호출 할 수 있을까요? no
